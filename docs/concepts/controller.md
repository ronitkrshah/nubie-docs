...
1. Scans the `controllers/` directory for valid controller classes.
2. Identifies classes decorated with `@ApiController()`, regardless of whether they are exported.
3. Derives the route prefix from the class name or a custom string.
4. Registers decorated methods as route handlers using metadata.
5. Applies middleware such as validation, authentication, and dependency injection based on additional decorators.
